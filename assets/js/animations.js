...
// Dedicated text choreography for the Solution section so every copy block reveals in sequence.
function initSolutionTextBlocksMotion() {
  ...
  const labelChip = section.querySelector('.solution-section__label-chip');
  const title = section.querySelector('.solution-section__title');
  const leadCopy = section.querySelector('.solution-card__lead-copy');
  const cardNumbers = gsap.utils.toArray(
    '.solution-card:not(.solution-card--lead) .solution-card__number',
    section
  );
  const cardDescriptions = gsap.utils.toArray(
    '.solution-card:not(.solution-card--lead) .solution-card__desc',
    section
  );
  const cardDetails = gsap.utils.toArray(
    '.solution-card:not(.solution-card--lead) .solution-card__detail',
    section
  );
  const summaryTitle = section.querySelector('.solution-section__summary-title');
  const capabilityTitles = gsap.utils.toArray('.solution-capability-item__title', section);
  const capabilityTags = gsap.utils.toArray('.solution-capability-item__tag', section);
  const docsLink = section.querySelector('.solution-section__docs-link');
